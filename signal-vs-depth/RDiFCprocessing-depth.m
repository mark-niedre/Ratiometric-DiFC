...
%uncomment below for 0.75mm deep trial3
%prom_fact = 3.8; %%parameter to describe peak prominence
%std_thresh is a multiplicative factor for excluding the current std.
%adjust std_thresh to 1.06 for 0.75mm deep trial 3 
std_thresh = 1.04;
...
if ~proc_flag && exist([fname '_proc.mat'], 'file') ~= 0
    ...
    % data array
  [data_bs, noise, peaks, thresh_curve, in_dat] = preProc(data, time, params, 'RelativeThresh', rel_thresh, 'StdThresh', std_thresh,'ProminenceFactor',prom_fact);
